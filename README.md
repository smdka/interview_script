...
* [Теория](#теория)
  * [ООП](#1-ооп)
  * [Java Core](#2-java-core)
  * [Spring Framework](#3-spring-framework)
  * [Hibernate](#4-hibernate)
  * [Базы данных](#5-базы-данных)
  * [WEB](#6-web)
  * [Микросервисы](#7-микросервисы)
* [Лайф-кодинг](#лайф-кодинг)
  * [Первая задача](#первая-задача)
  * [Вторая задача](#вторая-задача)
  * [Третья задача](#третья-задача)
...
#### 1.1 Что такое ООП? Плюсы и минусы ООП?

Ожидаемый ответ:

ООП (Объектно-ориентированное программирование) - это парадигма программирования, которая основана на концепции объектов
и классов. Объекты представляют собой экземпляры классов, которые содержат данные и методы для работы с этими данными.
Классы определяют структуру и поведение объектов.

Плюсы ООП:

* Модульность: ООП позволяет разбить программу на независимые модули (классы), которые можно легко изменять и расширять.
* Переиспользование кода: Классы можно использовать повторно в разных частях программы, что экономит время и усилия.
* Улучшенная читаемость: ООП позволяет писать более читаемый и понятный код, поскольку каждый класс имеет свою
* ответственность и функциональность.
* Легкость поддержки: ООП позволяет легко добавлять новые функции и исправлять ошибки, не затрагивая другие части
* программы.

Минусы ООП:

* Сложность: ООП может быть сложным для начинающих программистов, поскольку требует понимания объектов, классов и
* наследования.
* Низкая производительность: ООП может привести к снижению производительности, поскольку требует создания и управления
* объектами.
* Увеличенный размер кода: ООП может привести к увеличению размера кода, поскольку каждый класс имеет свою реализацию.

В целом, ООП является мощным инструментом для разработки сложных программ, но требует тщательного планирования и
проектирования.

#### 1.2 В чем основное отличие между абстрактным классом и интерфейсом?

Ожидаемый ответ:

Абстрактный класс - это класс, который не может быть инстанцирован и содержит как абстрактные, так и неабстрактные
методы.

Интерфейс - это контракт, который должен быть реализован классом, а также не может хранить состояние.

#### 1.3 Какие существуют типы связей между объектами в ООП?

Ожидаемый ответ:

**Ассоциация** – знание одного класса о другом. Например, класс Начальник может быть связан с классом Подчиненный отношением
ассоциации, чтобы передавать ему запросы. Если и Подчиненный должен обращаться к Начальнику – ассоциация называется
двунаправленной.

**Агрегация** и **композиция** – две разновидности ассоциации. Композиция подразумевает, что ассоциированный объект –
«подобъект», составная часть, не имеющая смысла без ссылающегося. На практике, например, отношение композиции появляется
когда мы выделяем часть свойств объекта в отдельную сущность.

Агрегация не подразумевает отношения владения. Это просто ссылка на другой, самостоятельный объект. Он может продолжить
функционировать, если ссылающийся объект прекратит существование. Например, класс-контейнер агрегирует хранимые
элементы.

#### 1.4 Что такое паттерны (шаблоны проектирования)? С какими ты сам сталкивался и в каких ситуациях?
...
#### 2.1.2 Как передается значение переменной в Java? По ссылке или по значению?

Ожидаемый ответ:

В Java переменные передаются по значению (call by value). Это означает, что при передаче переменной в метод копируется
ее значение, а не сама переменная.

Например, если у вас есть метод, который принимает целочисленное значение, и вы передаете ему переменную, то метод
получит копию значения этой переменной, а не саму переменную.

Однако, если вы передаете объект, то метод получит копию ссылки на этот объект, а не сам объект. Это означает, что метод
может изменить состояние объекта, но не может изменить саму ссылку.
...
#### 2.3.1 В чем разница между интерфейсами Comparable и Comparator?

Ожидаемый ответ:

**Comparable** и **Comparator** - это два интерфейса в Java, которые используются для сравнения объектов. Основное
различие
между ними заключается в том, что:

**Comparable** - это интерфейс, который реализуется классом, объекты которого нужно сравнивать. Он определяет метод
compareTo(), который сравнивает текущий объект с другим объектом того же класса.

**Comparator** - это интерфейс, который реализуется классом, который сравнивает объекты другого класса. Он определяет
метод
compare(), который сравнивает два объекта.

В общем, если вы хотите, чтобы объекты вашего класса могли сравниваться между собой, вы реализуете Comparable. Если вы
хотите сравнивать объекты другого класса, вы реализуете Comparator.
...
В Java, Stream представляет собой последовательность элементов, поддерживающую функциональные операции (например, map,
filter, reduce). Это позволяет работать с данными в более декларативном стиле, без необходимости явного использования
циклов или условных операторов.

Stream может быть создан из различных источников данных, таких как коллекции, массивы, файлы или базы данных.
...
* **Фантомное чтение (phantom reads)** — одна транзакция в ходе своего выполнения несколько раз выбирает множество
  записей по одним и тем же критериям. Другая транзакция в интервалах между этими выборками добавляет или удаляет записи, или
  изменяет столбцы некоторых записей, используемых в критериях выборки первой транзакции, и успешно заканчивается. В результате
...
UDP, User Datagram Protocol (Протокол пользовательских датаграмм) — протокол, который обеспечивает доставку без
...
**TCP** — ориентированный на соединение протокол, что означает необходимость «рукопожатия» для установки соединения
между двумя хостами. Как только соединение установлено, пользователи могут отправлять данные в обоих направлениях.

* Надёжность — TCP управляет подтверждением, повторной передачей и тайм-аутом сообщений. Производятся многочисленные
  попытки доставить сообщение. Если оно потеряется на пути, сервер вновь запросит потерянную часть. В TCP нет ни
  пропавших данных, ни (в случае многочисленных тайм-аутов) разорванных соединений.
* Упорядоченность — если два сообщения последовательно отправлены, первое сообщение достигнет приложения-получателя
  первым. Если участки данных приходят в неверном порядке, TCP отправляет неупорядоченные данные в буфер до тех пор,
  пока все данные не могут быть упорядочены и переданы приложению.
* Тяжеловесность — TCP необходимо три пакета для установки соединения перед тем, как отправить данные. TCP следит за
  надёжностью и перегрузками.
* Потоковость — данные читаются как поток байтов, не передается никаких особых обозначений для границ сообщения или
  сегментов.

**UDP** — более простой, основанный на сообщениях протокол без установления соединения. Протоколы такого типа не
устанавливают выделенного соединения между двумя хостами. Связь достигается путём передачи информации в одном
направлении от источника к получателю без проверки готовности или состояния получателя.

* Ненадёжность — когда сообщение посылается, неизвестно, достигнет ли оно своего назначения — оно может потеряться по
  пути. Нет таких понятий как подтверждение, повторная передача, тайм-аут.
* Неупорядоченность — если два сообщения отправлены одному получателю, то порядок их достижения цели не может быть
  предугадан.
* Легковесность — никакого упорядочивания сообщений, никакого отслеживания соединений и т. д. Это лишь транспортный
  уровень.
* Датаграммы — пакеты посылаются по отдельности и проверяются на целостность только если они прибыли. Пакеты имеют
  определенные границы, которые соблюдаются после получения, то есть операция чтения на получателе выдаст сообщение
  таким, каким оно было изначально послано.
* Отсутствие контроля перегрузок — для приложений с большой пропускной способностью существует шанс вызвать коллапс
...
* Поставщики (серверы), которые ожидают соединения для получения запроса, производят необходимые действия и возвращают
  обратно сообщение с результатом.

Для идентификации ресурсов HTTP использует глобальные URI.

HTTP не сохраняет своего состояния. Это означает отсутствие сохранения промежуточного состояния между парами
«запрос-ответ».
...
* Тело сообщения (message body) — непосредственно данные сообщения. Обязательно должно отделяться от заголовков пустой
  строкой.
...
по протоколу HTTPS, «упаковываются» в криптографический протокол SSL или TLS, что обеспечивает защиту от атак,
основанных на прослушивании сетевого соединения (при условии, что будут использоваться шифрующие средства и сертификат
...
* Парсинг URL: Браузер разбирает введенный URL на его компоненты, такие как протокол (http или https), домен, путь и
  параметры.
* Резолвинг домена: Браузер отправляет запрос на сервер имен (DNS) для получения IP-адреса домена. DNS-сервер возвращает
  IP-адрес, который соответствует домену.
* Установление соединения: Браузер устанавливает соединение с сервером по протоколу TCP/IP, используя полученный
  IP-адрес.
* Отправка запроса: Браузер отправляет HTTP-запрос на сервер, который включает в себя метод запроса (GET, POST, PUT,
  DELETE и т. д.), заголовки и тело запроса.
* Обработка запроса на сервере: Сервер получает запрос и обрабатывает его, используя соответствующий обработчик
  запросов. Обработчик запросов может быть написан на любом языке программирования, таком как PHP, Python, Ruby и т. д.
...
* Отображение содержимого: Браузер отображает содержимое ответа, которое может быть HTML-страницей, изображением, файлом
  и т. д.

* Весь этот процесс происходит очень быстро, обычно за несколько секунд. Однако, если сервер находится далеко или имеет
  высокую нагрузку, время ответа может быть больше.

Кроме того, современные браузеры также используют различные оптимизации, такие как:

* Кеширование: Браузер хранит часто используемые ресурсы, такие как изображения и скрипты, в кеше, чтобы избежать
  повторных запросов на сервер.
...
Синхронное и асинхронное взаимодействие между микросервисами - это два различных подхода к обмену данными между
сервисами в микросервисной архитектуре.

Синхронное взаимодействие

Синхронное взаимодействие между микросервисами означает, что один сервис отправляет запрос другому сервису и ожидает
ответа перед тем, как продолжить свою работу. Это означает, что сервис, отправивший запрос, блокируется до тех пор, пока
не получит ответ от другого сервиса.
...
Асинхронное взаимодействие между микросервисами означает, что один сервис отправляет запрос другому сервису, но не
ожидает ответа. Вместо этого, сервис, отправивший запрос, продолжает свою работу, а ответ от другого сервиса
...
* В общем, синхронное взаимодействие подходит для систем, где необходимо обеспечить сильную связь между сервисами, а
  асинхронное взаимодействие подходит для систем, где необходимо обеспечить высокую производительность и слабую связь
  между сервисами.
...
* Обработка ошибок: Брокер сообщений обрабатывает ошибки доставки сообщений и позволяет микросервисам повторно
  отправлять
...
#### 7.5.1 Что такое топики и очереди в брокере. Для чего они?
...
**Топик** - это понятие, которое позволяет отправлять сообщения по определённой теме или категории. Топики позволяют
отправлять сообщения нескольким потребителям, которые подписаны на ту же тему. Когда сообщение отправляется в топик, оно
доставляется всем потребителям, которые подписаны на этот топик.

Основное различие между очередями и топиками заключается в том, что очередь предназначена для обработки сообщений в
порядке их поступления, а топик предназначен для доставки сообщений нескольким потребителям по определённой теме.
...
* Client-Server (Клиент-Сервер): Взаимодействие между клиентом и сервером осуществляется через стандартизированный
  интерфейс.
* Stateless (Без сохранения состояния): Каждый запрос от клиента к серверу должен содержать всю необходимую информацию
  для его обработки. Сервер не должен сохранять состояние между запросами.
...
##### 7.8.1 Что такое dockerfile, docker image и контейнер

##### 7.8.2 Что такое docker compose?
...
Нужно написать такой SQL-запрос, который выведет id и имя сотрудников, работающих в компании с лета 2021 года, в
отделе "Цифровые решения" и получающих зарплату меньше 100 000 рублей.
